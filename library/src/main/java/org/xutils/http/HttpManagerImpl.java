...
public final class HttpManagerImpl implements HttpManager {
    ...
    public <T> Callback.Cancelable request(HttpMethod method, RequestParams entity, Callback.CommonCallback<T> callback) {
        ...
        synchronized (lock) {
            if (HttpTask.IS_JUDGE_REPEAT) {
                if ((callback instanceof Callback.CacheCallback) && HttpTask.hasTask(entity.toString())) {
                    entity.addQueryStringParameter("xUtils", String.valueOf(System.currentTimeMillis()));
                }
            }

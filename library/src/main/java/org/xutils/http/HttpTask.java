...
public class HttpTask<ResultType> extends AbsTask<ResultType> implements ProgressHandler {
    ...
    public HttpTask(RequestParams params, Callback.Cancelable cancelHandler,
        ...
        if (IS_JUDGE_REPEAT) {
            synchronized (BLOCK_JUDGE_REPEAT) {
                mHttpTaskList.add(this);
            }
        }
    ...
    private static final Object BLOCK_JUDGE_REPEAT = new Object();

    private static List<HttpTask> mHttpTaskList = new ArrayList<>();

    private String getRequestUri() {
        return params.toString();
    }

    public static boolean hasTask(String uri) {
        synchronized (BLOCK_JUDGE_REPEAT) {
            if (TextUtils.isEmpty(uri)) {
                return false;
            }
            int size = mHttpTaskList.size();
            for (int i = 0; i < size; i++) {
                HttpTask httptask = mHttpTaskList.get(i);
                if (uri.equals(httptask.getRequestUri())) {
                    return true;
                }
            }
            return false;
    ...
    public static boolean IS_JUDGE_REPEAT = true;
